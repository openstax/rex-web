...
@pytest.mark.asyncio
async def test_redirect_to_osweb_404_when_book_is_incorrect(
...
@pytest.mark.asyncio
async def test_redirect_to_osweb_404_when_book_does_not_exist(
    chrome_page, base_url, book_slug, page_slug
):
...
@pytest.mark.parametrize(
    "book_slug, page_slug", [("astronomy-2e", "9-3333-impact-craters")]
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize("book_slug, page_slug", [("astronomy-2e", "")])
@pytest.mark.asyncio
async def test_missing_page(chrome_page, base_url, book_slug, page_slug):
    """User gets the Uh-oh, no page here error"""

    # GIVEN: A missing page slug
