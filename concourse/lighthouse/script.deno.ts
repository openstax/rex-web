...
if (!process.env.LIGHTHOUSE_PAGES ||
    !process.env.LIGHTHOUSE_PREVIOUS_REPORT_DIR ||
    !process.env.LIGHTHOUSE_REPORT_DIR) {
  throw new Error(
    'You must define the LIGHTHOUSE_PAGES, LIGHTHOUSE_PREVIOUS_REPORT_DIR and LIGHTHOUSE_REPORT_DIR env variables'
  );
}

const pages: string[] = JSON.parse(process.env.LIGHTHOUSE_PAGES);
const previousReportDir = process.env.LIGHTHOUSE_PREVIOUS_REPORT_DIR;
const reportDir = process.env.LIGHTHOUSE_REPORT_DIR;
...
await asyncPool(1, pages, async(pageUrl) => {
  ...
  const targets = await new Promise<ScoreTargets | undefined>(
    (resolve) => readFile(
      `${process.env.LIGHTHOUSE_PREVIOUS_REPORT_DIR}/${filename}`,
      { encoding: 'utf8' },
