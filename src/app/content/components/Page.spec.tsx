...
describe("Page", () => {
  ...
  beforeEach(() => {
    ...
    ((allImagesLoaded as any) as jest.SpyInstance).mockReturnValue(
      Promise.resolve()
    );
  ...
  describe("Content tweaks for generic styles", () => {
    ...
    it("wraps note titles in a <header> and contents in a <section>", async () => {
      expect(
        await htmlHelper(
          '<div data-type="note"><div data-type="title">TT</div><p>BB</p></div>'
        )
      ).toEqual(
        '<div data-type="note" class="ui-has-child-title">' +
    ...
    it("adds a label to the note when present", async () => {
      expect(
        await htmlHelper(
          '<div data-type="note" data-label="LL"><div data-type="title">notetitle</div></div>'
        )
      ).toEqual(
        '<div data-type="note" data-label="LL" class="ui-has-child-title">' +
    ...
    it("converts notes without titles", async () => {
      expect(
        await htmlHelper('<div data-type="note">notewithouttitle</div>')
      ).toEqual(
        '<div data-type="note"><header></header><section>notewithouttitle</section></div>'
      );
    });

    it("moves figure captions to the bottom", async () => {
      expect(
        await htmlHelper("<figure><figcaption>CC</figcaption>FF</figure>")
      ).toEqual(
        '<figure class="ui-has-child-figcaption">FF<figcaption>CC</figcaption></figure>'
      );
    });

    it("numbers lists that have a start attribute", async () => {
      expect(await htmlHelper('<ol start="123"><li>item</li></ol>')).toEqual(
        '<ol start="123" style="counter-reset: list-item 123"><li>item</li></ol>'
      );
    });
    ...
    it("adds prefix to list items", async () => {
      expect(
        await htmlHelper(
          '<ol data-mark-prefix="[mark-prefix]"><li>item</li></ol>'
        )
      ).toEqual(
        '<ol data-mark-prefix="[mark-prefix]"><li data-mark-prefix="[mark-prefix]">item</li></ol>'
      );
    });
    ...
    it("adds a suffix to list items", async () => {
      expect(
        await htmlHelper(
          '<ol data-mark-suffix="[mark-suffix]"><li>item</li></ol>'
        )
      ).toEqual(
        '<ol data-mark-suffix="[mark-suffix]"><li data-mark-suffix="[mark-suffix]">item</li></ol>'
      );
    });
    ...
    it("updates content self closing tags", async () => {
      expect(
        await htmlHelper(
          `<strong data-somethin="asdf"/>asdf<iframe src="/someplace"/>`
        )
      ).toEqual(
    ...
    describe("solutions", () => {
      ...
      it("are not transformed if already formatted correctly", async () => {
        expect(
          ...
          "<div data-type=\\"exercise\\" id=\\"exercise1\\" data-element-type=\\"check-understanding\\" class=\\"ui-has-child-title\\"><header><h3 class=\\"os-title\\"><span class=\\"os-title-label\\">Check Your Understanding</span></h3></header><section>
                      
                      <div data-type=\\"problem\\" id=\\"problem1\\"><div class=\\"os-problem-container\\">
                        <p id=\\"paragraph1\\">blah blah blah</p>
                      </div></div>
                      <details data-type=\\"solution\\" id=\\"exercise1\\" data-element-type=\\"check-understanding\\" aria-label=\\"Show/Hide Solution\\">
                  <summary class=\\"btn-link ui-toggle\\" title=\\"Show/Hide Solution\\" data-content=\\"Show/Hide Solution\\"></summary>
                  <section class=\\"ui-body\\" role=\\"alert\\"><h3 class=\\"os-title\\"><span class=\\"os-title-label\\">Check Your Understanding</span></h3>
                        <div data-type=\\"problem\\" id=\\"problem1\\"><div class=\\"os-problem-container\\">
                          <p id=\\"paragraph1\\">blah blah blah</p>
                        </div></div>
                        <details data-type=\\"solution\\" id=\\"fs-id2913818\\" data-print-placement=\\"here\\" aria-label=\\"Show/Hide Solution\\">
...
<div data-type="exercise" id="exercise1" data-element-type="check-understanding" class="ui-has-child-title"><header><h3 class="os-title"><span class="os-title-label">Check Your Understanding</span></h3></header></section>
            ...
            <details data-type="solution" id="fs-id2913818" data-print-placement="here">
              <h4 data-type="title" class="solution-title"><span class="os-text">Solution</span></h4>
              <div class="os-solution-container">
                <p id="paragraph2">answer answer answer.</p>
              </div>
        ...
        const solutionSection = pageElement.querySelector(
          "#exercise1 .ui-body"
        );
      ...
      it("doesn't throw when badly formatted", async () => {
        ...
        const button = pageElement.querySelector(
          '[data-type="solution"] > .ui-toggle'
        );
        const solution = pageElement.querySelector(
          'details[data-type="solution"]'
        );
        ...
        Object.defineProperty(button.parentElement, "parentElement", {
          value: null,
          writable: true
        });
      ...
      it("is automatically expanded if it contains a link anchor", async () => {
        ...
        const details = pageElement.querySelector(
          '[data-type="solution"]'
  ...
  it("updates content link with new hrefs", async () => {
    ...
    const [firstLink, secondLink] = Array.from(
      root.querySelectorAll("#main-content a")
    );
    ...
    expect(firstLink.getAttribute("href")).toEqual(
      "books/book-slug-1/pages/page-title"
    );
  ...
  it("interceptes clicking content links", async () => {
    ...
    dispatch.mockReset();
    const [firstLink, secondLink, thirdLink] = Array.from(
      root.querySelectorAll("#main-content a")
    );
  ...
  it("interceptes clicking links that failed due to reference loading error", async () => {
    ...
    expect(spyAlert).toHaveBeenCalledWith(
      "This link is broken because of a cross book content loading issue"
    );
  ...
  it("passes search when clicking archive links", async () => {
    routes.content.getSearch = jest
      .fn()
      .mockReturnValue("archive=some-content");
    ...
    expect(dispatch).toHaveBeenCalledWith(
      receivePage(expect.objectContaining({ references }))
    );
  });
  ...
  it("renders error modal for different search results", async () => {
    ...
    const errorModalCloseButton = root.querySelector(
      "[data-testid=banner-body] button"
    );
  ...
  it("refresh error modal for different search results if they are of the same type", async () => {
    ...
    const dateMock = jest.spyOn(Date, "now").mockReturnValue(1);
    ...
    expect(dispatch).toHaveBeenCalledWith(
      addToast(toastMessageKeys.search.failure.nodeNotFound, {
        destination: "page"
      })
    );
    dispatch.mockClear();
  ...
  it("renders error modal for highlight scroll target when it cant find a highlight - only once", async () => {
    const mockScrollTarget = `target=${JSON.stringify({
      type: "highlight",
      id: "some-id"
    })}`;
    ...
    expect(dispatch).toHaveBeenCalledWith(
      addToast(toastMessageKeys.highlights.failure.search, {
        destination: "page"
      })
    );
    dispatch.mockClear();
  ...
  it("waits for images to load before scrolling to a target element", async () => {
    ...
    const someHashPage = {
      abstract: "",
      content:
        '<div style="height: 1000px;"></div><img src=""><div id="somehash"></div>',
      id: "adsfasdf",
      revised: "2018-07-30T15:58:45Z",
      slug: "mock-slug",
      title: "qerqwer"
  ...
  it("renders <PageNotFound> component", () => {
    jest.spyOn(select, "pageNotFound").mockReturnValue(true);
